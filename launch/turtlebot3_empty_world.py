#!/usr/bin/env python3

from launch import LaunchDescription
from launch.actions import DeclareLaunchArgument, IncludeLaunchDescription
from launch.substitutions import LaunchConfiguration, Command, PathJoinSubstitution, TextSubstitution
from launch_ros.actions import Node
from launch.launch_description_sources import PythonLaunchDescriptionSource
from ament_index_python.packages import get_package_share_directory
import os


def generate_launch_description():
    model = LaunchConfiguration('model')
    x_pos = LaunchConfiguration('x_pos')
    y_pos = LaunchConfiguration('y_pos')
    z_pos = LaunchConfiguration('z_pos')

    # Get package paths
    gazebo_ros_pkg = get_package_share_directory('gazebo_ros')
    turtlebot3_gazebo_pkg = get_package_share_directory('turtlebot3_gazebo')
    turtlebot3_description_pkg = get_package_share_directory('turtlebot3_description')

    # Set file paths
    world_path = os.path.join(turtlebot3_gazebo_pkg, 'worlds', 'empty.world')
    urdf_file = os.path.join(turtlebot3_description_pkg, 'urdf', f'turtlebot3_{model.perform(None)}.urdf.xacro')

    return LaunchDescription([
        # Declare arguments
        DeclareLaunchArgument('model', default_value=TextSubstitution(text='waffle_pi')),
        DeclareLaunchArgument('x_pos', default_value=TextSubstitution(text='0.0')),
        DeclareLaunchArgument('y_pos', default_value=TextSubstitution(text='0.0')),
        DeclareLaunchArgument('z_pos', default_value=TextSubstitution(text='0.0')),

        # Launch Gazebo with the empty world
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(gazebo_ros_pkg, 'launch', 'gazebo.launch.py')
            ),
            launch_arguments={'world': world_path}.items()
        ),

        # Robot state publisher
        Node(
            package='robot_state_publisher',
            executable='robot_state_publisher',
            output='screen',
            parameters=[{
                'robot_description': Command([
                    'xacro ', urdf_file
                ])
            }]
        ),

        # Spawn the robot into Gazebo
        Node(
            package='gazebo_ros',
            executable='spawn_entity.py',
            arguments=[
                '-entity', f'turtlebot3_{model.perform(None)}',
                '-topic', 'robot_description',
                '-x', x_pos,
                '-y', y_pos,
                '-z', z_pos
            ],
            output='screen'
        )
    ])
